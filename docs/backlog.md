...
Status: Maintenance 1.1, 1.2, 1.3 и 1.4 — complete (все BL-* done)  
Owner: @Ivan  
Created: 2026-05-25  
Last updated: 2026-08-31 (Maintenance 1.4 — спринт M закрыт полностью)
...
| BL-1108 | Привести раздел «Структура проекта» в README к факту | GAP-CR-032 | P3 | **done** | S |
| BL-1109 | Актуализировать `devplan.md` / `devplan_ru.md` | GAP-CR-033 | P3 | **done** | M |
| BL-1110 | Не гасить `typer.Exit` широким `except` в сценариях | GAP-CR-034 | P1 | **done** | S |
...
Состояние после блока 3 (BL-1105 + BL-1106, 2026-08-31): `execute_download` — 112 строк, `download_command.py` — 294 строки вместо 999, сценарии вынесены в отдельные модули. Тесты: **110 passed, 2 skipped**; `ruff check .` чисто.

Состояние после BL-1110 (2026-08-31): остановка пользователем больше не гасится сценариями. Тесты: **119 passed, 2 skipped**.

Спринт закрыт (BL-1108, BL-1109, 2026-08-31): README и `devplan*.md` приведены к состоянию кода. Все 10 задач спринта K — `done`.

---

## Спринт L — Maintenance 1.3 (открыт, 2026-08-31)

Источник: [ревью техдолга 2026-08-31](./gaps/tech_debt_2026-08-31.md), дизайн решений — [design_tech_debt_2026-08-31.md](./design_tech_debt_2026-08-31.md).

| ID | Задача | Gap | Priority | Status | Est |
|----|--------|-----|----------|--------|-----|
| BL-1201 | Закрывать родительскую запись истории для плейлиста | GAP-CR-035 | P1 | **done** | S |
| BL-1202 | Единственный читатель клавиатуры в `PauseController` | GAP-CR-036 | P1 | **done** | M |
| BL-1203 | Проверка типов (mypy) в CI | GAP-CR-037 | P2 | **done** | M |
| BL-1204 | Измерение покрытия тестами с порогом | GAP-CR-038 | P2 | **done** | S |
| BL-1205 | Решение и настройка защиты ветки `main` | GAP-CR-039 | P2 | **done** | S |
| BL-1206 | Решение по интеграционным тестам | GAP-CR-040 | P3 | **done** | S |
| BL-1207 | Решение по JSONL и SQLite | GAP-CR-009 | P3 | **done** | M |

Подробные задачи — в [Группе 12](#группа-12--maintenance-13-ревью-техдолга-2026-08-31). Порядок и зависимости — в разделе «Порядок внедрения» дизайн-дока.

Состояние на момент открытия спринта: 121 тест (119 passed, 2 skipped), `ruff check .` чисто, CI зелёный на обеих платформах. BL-1201 и BL-1202 — подтверждённые дефекты, остальное — ворота качества и решения владельца.

Состояние после блока дефектов (BL-1201 + BL-1202, 2026-08-31): 127 тестов (125 passed, 2 skipped), `ruff check .` чисто.

Состояние после ворот качества (BL-1203 + BL-1204, 2026-08-31): 129 тестов (127 passed, 2 skipped), покрытие 68% при пороге 66%, mypy зелёный под тремя платформенными моделями. Побочно исправлен пользовательский баг с `typer.BadParameter`, найденный первым же прогоном mypy. Осталось: BL-1205, BL-1206, BL-1207 — все требуют решения владельца.

---

## Спринт M — Maintenance 1.4 (закрыт, 2026-08-31)

Источник: не гэп-ревью, а два практических повода — жалоба на воспроизводимость
скачанных MP4 и устаревшая версия yt-dlp (2026.3.17 при текущей 2026.8.19).

| ID | Задача | Gap | Priority | Status | Est |
|----|--------|-----|----------|--------|-----|
| BL-1301 | Приоритет H.264/AVC1 в селекторах формата MP4 | — | P1 | **done** | M |
| BL-1302 | Обновление зависимостей, включая yt-dlp | — | P2 | **done** | S |

Подробные задачи — в [Группе 13](#группа-13--maintenance-14-совместимость-и-зависимости-2026-08-31).

Состояние на момент открытия спринта: 129 тестов (127 passed, 2 skipped), `ruff check .`
чисто, mypy зелёный под тремя платформенными моделями, покрытие 68% при пороге 66%.

Состояние после закрытия спринта (2026-08-31): 132 теста (**130 passed, 2 skipped**), покрытие 67.63%
при пороге 66%, `ruff check .` чисто (ruff 0.16.5), mypy зелёный под win32/linux/darwin
(mypy 2.3.1). Проверено на живой сети: `ytd info` отдаёт 16 форматов вместо 11,
`ytd download` собирает файл, `ffprobe` подтверждает `codec_name=h264`.
...
| 12 | Maintenance 1.3 (ревью техдолга 2026-08-31) | 7 | 0 | 2 | 3 | 2 |
...
5. [x] Отдельное решение владельца: очищать ли историю коммитов (`git filter-repo` + force-push).
   **Решение (2026-08-31): историю не переписываем.** Ссылки остаются в прошлых ревизиях;
   содержимое безобидное (14 ссылок на видео с подписями), учётных данных в публичной
   истории нет — проверено поиском по всем ревизиям, а переписывание истории меняет все
   SHA и ломает ссылки в закрытых PR. Уточнение: репозиторий **публичный** — исходная
   формулировка про приватность была ошибочной, решение перепроверено на верных вводных.
...
- **Priority:** P3 | **Status:** done (2026-08-31) | **Estimate:** S
...
**Реализовано (2026-08-31).** Из дерева убраны `data/.gitkeep` и `downloads/.gitkeep`, добавлены реально существующие `config.example.yaml` и `urls.example.txt`, уточнено назначение `workflows/`. Ниже блока — примечание, что `downloads/` и `data/` создаются автоматически по путям из конфигурации.
...
- **Priority:** P3 | **Status:** done (2026-08-31) | **Estimate:** M
...
**Реализовано (2026-08-31).** Обновлены: шапка (версия Maintenance 1.2, дата), «Реализовано» (per-video режим, субтитры, cookies, история SQLite, паузы, Python 3.14, uv, 121 тест, CI-матрица), «Ограничения сейчас» (убраны реализованные пункты, уточнено, что возобновление — рестарт yt-dlp с `continuedl`), диаграмма архитектуры (`workflows` вместо `Interactive`, команда `history` вместо несуществующей `interactive`), «Технический долг» (актуальные пункты + перечень закрытого). R1 помечен выполненным, R2 — закрытым иначе, с фактической структурой `workflows/` вместо предлагавшегося `services/`. Также поправлены статус фичи 1.1 и версия Python в примере Dockerfile. Обе версии — 1328 строк, заголовки на совпадающих номерах строк.
...
**Реализовано (2026-08-31).** `except (typer.Exit, typer.Abort): raise` в трёх сценариях. `typer.Abort` (это `click.Abort`) добавлен по той же причине: тоже наследуется от `RuntimeError` и означает отмену (Ctrl+C или EOF в диалоге). В `execute_download` `typer.Abort` обрабатывается наравне с `KeyboardInterrupt` — вместо «✗ Ошибка: » с пустым текстом выводится «✗ Прервано пользователем», код 1. Тесты: 6 параметризованных «сценарий не гасит остановку» + 3 «обычная ошибка по-прежнему даёт fallback» (чтобы исправление не пережало откат). Всего 119 passed, 2 skipped.

---

## Группа 12 — Maintenance 1.3 (ревью техдолга 2026-08-31)

Цель: закрыть два подтверждённых дефекта, доставшихся с прошлого цикла, и достроить систему качества так, чтобы подобные дефекты находились автоматикой, а не чтением кода.

Дизайн решений — [design_tech_debt_2026-08-31.md](./design_tech_debt_2026-08-31.md), разделы D-1 … D-7.

### BL-1201 — Закрывать родительскую запись истории для плейлиста

- **Gap:** [GAP-CR-035](./gaps/tech_debt_2026-08-31.md#gap-cr-035--висящая-запись-in_progress-для-плейлиста) | **Design:** D-1
- **Priority:** P1 | **Status:** done (2026-08-31) | **Estimate:** S

**Описание.** `in_progress` пишется по ключу `opts.url`, а `success` — по ключам элементов плейлиста, поэтому родительская строка остаётся незакрытой навсегда. Подтверждено экспериментом: после успешной загрузки плейлиста из двух элементов в базе три строки, одна из них `in_progress`.

**Шаги.**
1. Добавить `Downloader._record_playlist_parent(info, opts, status=...)`, записывающий плоское событие по `opts.url` с заголовком и `playlist_id`.
2. Вызвать его после успеха (`status="success"`) и в ветке ошибки (`status="failed"`).
3. Тесты: `test_playlist_parent_row_is_closed_after_success`, регрессия на одиночное видео.

**Критерии приёмки.**
- После успешной загрузки плейлиста строк со статусом `in_progress` по его URL нет.
- Повторный запуск того же плейлиста спрашивает про уже скачанное, а не про «незавершённую загрузку».
- `tests/test_downloader_history.py` проходит без изменений.

**Реализовано (2026-08-31).** `Downloader._close_parent_record` сравнивает нормализованный ключ `opts.url` с ключами финальных событий и при расхождении пишет событие по самому URL (для плейлиста — с заголовком и `playlist_id`); вызывается и на успехе, и при ошибке.

**Область оказалась шире, чем в исходном описании гэпа.** Регрессионный тест показал, что `normalize_history_id` сводит URL и `id` к общему ключу только на YouTube; на VK и других площадках они различаются, поэтому висящая строка возникала и для одиночных видео. Решение сделано общим — оно исходит из расхождения ключей, а не из признака «плейлист». Тесты: `test_playlist_parent_row_is_closed_after_success`, `test_single_video_parent_row_is_closed_when_keys_differ`, `test_youtube_single_video_has_one_row`.

---

### BL-1202 — Единственный читатель клавиатуры в `PauseController`

- **Gap:** [GAP-CR-036](./gaps/tech_debt_2026-08-31.md#gap-cr-036--гонка-за-клавиатурным-вводом-при-возобновлении) | **Design:** D-2
- **Priority:** P1 | **Status:** done (2026-08-31) | **Estimate:** M

**Описание.** Слушатель пауз и `wait_if_paused` одновременно читают один поток ввода; символ, доставшийся слушателю и не равный `pause_key`, молча выбрасывается. Из-за этого `resume_key` срабатывает не всегда.

**Шаги.**
1. Добавить `self._resume_requested = threading.Event()`.
2. Научить `_handle_listener_key` распознавать `resume_key` (и Enter) при активной паузе.
3. Переписать `wait_if_paused` на ожидание события; сохранить prompt-fallback при не-TTY **и** при незапущенном слушателе.
4. Удалить `_wait_for_resume_windows` и `_wait_for_resume_unix`.
5. Тесты: событие возобновления, игнор `resume_key` вне паузы, выход из ожидания по событию, fallback без слушателя.

**Критерии приёмки.**
- Клавиатуру читает ровно один потребитель.
- `resume_key` возобновляет загрузку с первого раза.
- `tests/test_intra_video_pause.py` проходит без изменений; `tests/test_pause.py` обновлён под новую схему и зелёный на обеих платформах.

**Реализовано (2026-08-31).** Слушатель распознаёт `resume_key` и Enter при активной паузе и выставляет `threading.Event`; `wait_if_paused` ждёт событие. `_wait_for_resume_windows` и `_wait_for_resume_unix` удалены — вместе с ними исчезли платформенные ветки чтения из режима ожидания. Fallback на prompt теперь срабатывает и когда слушатель не запущен, а не только при не-TTY: иначе ожидание события повисло бы навсегда. Пять новых тестов, win32-only проверка из BL-1101 заменена платформенно-независимой.

---

### BL-1203 — Проверка типов (mypy) в CI

- **Gap:** [GAP-CR-037](./gaps/tech_debt_2026-08-31.md#gap-cr-037--нет-проверки-типов-в-ci) | **Design:** D-3
- **Priority:** P2 | **Status:** done (2026-08-31) | **Estimate:** M

**Описание.** Код почти полностью аннотирован, но аннотации никем не проверяются; существующие `# type: ignore` не валидируются.

**Шаги.**
1. `mypy>=1.11` в dev-группу, `[tool.mypy]` в `pyproject.toml` (не строгий старт, `files = ["ytd"]`, `ignore_missing_imports` для `yt_dlp.*`).
2. Прогнать локально, исправить настоящие несоответствия, оставшиеся подавить точечно с комментарием.
3. Добавить шаг `uv run mypy` в джобу `lint` (платформенно-независимо).
4. Описать локальный запуск в `DEVELOPMENT.md`.

**Критерии приёмки.** Шаг проверки типов в CI зелёный; глобальных отключений правил нет; `warn_unused_ignores` включён и не срабатывает.

**Примечание.** Если объём ошибок велик — допустимо сузить `files` и расширять отдельными задачами, зафиксировав это здесь явно.

**Реализовано (2026-08-31).** Сужать `files` не потребовалось: первый прогон дал 14 ошибок, все исправлены.

Среди них — **реальный пользовательский баг**: `typer.BadParameter(..., param_name=...)` при отсутствии такого параметра у `click.BadParameter`. Команды `ytd history --since <кривая дата>` и `ytd history export --format xml` падали с сырым `TypeError` вместо понятного сообщения (воспроизведено на реальном CLI). Исправлено на `param_hint`, добавлены два теста.

Остальное: 6 устаревших `# type: ignore`, неточный тип в `list_downloads`, платформенные импорты `termios`/`tty`/`winreg` без guard'ов. Вместо подавлений добавлены проверки `if sys.platform != "win32": return`, которые mypy понимает сам. Модель darwin выявила слишком широкую аннотацию `_detect_mac_browser`.

CI прогоняет три платформенные модели (`win32`, `linux`, `darwin`) из джобы `lint` — иначе половина платформенного кода осталась бы вне анализа, а результат зависел бы от ОС раннера.

---

### BL-1204 — Измерение покрытия тестами с порогом

- **Gap:** [GAP-CR-038](./gaps/tech_debt_2026-08-31.md#gap-cr-038--нет-метрики-покрытия-тестами) | **Design:** D-4
- **Priority:** P2 | **Status:** done (2026-08-31) | **Estimate:** S

**Описание.** Неизвестно, какая доля кода выполняется тестами; после декомпозиции `execute_download` это особенно важно — девять новых модулей.

**Шаги.**
1. `pytest-cov>=5` в dev-группу, `[tool.coverage.*]` в `pyproject.toml` (`branch = true`, исключения для `TYPE_CHECKING`).
2. Измерить текущее покрытие и записать значение в эту задачу.
3. Поставить `--cov-fail-under` на 1–2 пункта ниже измеренного; включить в шаг тестов на ubuntu.
4. Описать локальный запуск с `term-missing` в `DEVELOPMENT.md`.

**Критерии приёмки.** Покрытие печатается в логе CI; падение ниже порога роняет сборку; порог обоснован измерением, а не выбран произвольно.

**Реализовано (2026-08-31).** Измеренное покрытие — **68.04%** (branch-режим), порог `--cov-fail-under=66`. Считается только на ubuntu: платформенные ветки дают разный результат на двух ОС, и общий порог был бы хрупким.

Отчёт сразу дал карту непокрытого — вход для будущих задач: `network.py` 21%, `playlist_resume.py` 33%, `info_fetch.py` 36%, `interactive.py` 40%, `history_prompts.py` 45%. Это интерактивные ветки, требующие эмуляции диалогов. Ядро покрыто лучше: `context.py` и `types.py` 100%, `download_one.py` и `single_video.py` 93%, `config.py` 89%.

---

### BL-1205 — Решение и настройка защиты ветки `main`

- **Gap:** [GAP-CR-039](./gaps/tech_debt_2026-08-31.md#gap-cr-039--main-не-защищён-обязательными-проверками) | **Design:** D-5
- **Priority:** P2 | **Status:** done (2026-08-31) | **Estimate:** S
- **Зависимости:** BL-1203, BL-1204 (список обязательных проверок должен быть окончательным)

**Описание.** CI не блокирует ничего: прямой push в `main` проходит независимо от результата. За Maintenance 1.2 так ушло шесть коммитов.

**Шаги.**
1. [ ] Решение владельца: A — защита + PR-поток (рекомендуется), B — только запрет force-push, C — оставить как есть.
2. При выборе A: правило для `main` с обязательными `test (ubuntu-latest)`, `test (windows-latest)`, `lint`; запрет force-push и удаления ветки.
3. Описать рабочий поток в `DEVELOPMENT.md`.

**Критерии приёмки.** Решение принято и зафиксировано здесь с обоснованием; при выборе A — правило включено и поток описан.

**Примечание.** Задача не меняет код: это настройка в интерфейсе GitHub плюс документация.

**Решение (2026-08-31): вариант A — защита с обязательными проверками и PR-потоком.** Обоснование: за два цикла сопровождения в `main` ушло девять прямых пушей, каждый проверялся уже постфактум; CI сообщал, но ничего не гарантировал. Правило: обязательны `test (ubuntu-latest)`, `test (windows-latest)`, `lint`; требуется PR перед merge; force-push и удаление ветки запрещены; линейная история; ревью не требуется (работа одиночная). `enforce_admins` **включён**: проверка прямым push показала, что без него правило не связывает владельца и защита остаётся декоративной. Escape hatch — явное временное снятие через `gh api`, команды в `DEVELOPMENT.md`.

---

### BL-1206 — Решение по интеграционным тестам

- **Gap:** [GAP-CR-040](./gaps/tech_debt_2026-08-31.md#gap-cr-040--интеграционные-тесты-не-выполняются-нигде) | **Design:** D-6
- **Priority:** P3 | **Status:** done (2026-08-31) | **Estimate:** S

**Описание.** Два теста включаются переменной `YTD_IT_URL` и не выполнялись ни разу за два цикла; в отчёте видны как `2 skipped`, создавая видимость покрытия.

**Шаги.**
1. [ ] Решение: A — оставить ручным инструментом и задокументировать (рекомендуется), B — запускать по расписанию отдельной неблокирующей джобой, C — удалить.
2. При выборе A: раздел «Интеграционные тесты» в `DEVELOPMENT.md` с командой запуска и объяснением, почему они не в CI.

**Критерии приёмки.** Решение зафиксировано; строка `skipped` в обычном прогоне объяснена в документации.

**Решение (2026-08-31): вариант A — оставить ручным инструментом.** В `DEVELOPMENT.md` добавлен раздел с командами запуска (PowerShell и bash), описанием проверяемого (`ytd info --json`, `ytd download --dry-run`) и объяснением, почему их нет в CI: результат зависит от доступности площадки и anti-bot защиты, регулярные прогоны краснели бы не по вине кода.

---

### BL-1207 — Решение по JSONL и SQLite

- **Gap:** [GAP-CR-009](./gaps/tech_debt_2026-08-31.md#gap-cr-009-перенесён--jsonl-и-sqlite-как-два-источника-правды) | **Design:** D-7
- **Priority:** P3 | **Status:** done (2026-08-31) | **Estimate:** M

**Описание.** Метаданные пишутся в два хранилища с разной схемой. С момента прошлого решения появился `ytd history export --format jsonl`, закрывающий исходную потребность в построчном архиве.

**Шаги.**
1. [ ] Решение: A — оставить как есть, B — сделать JSONL опциональным и выключенным по умолчанию (рекомендуется), C — отказаться полностью.
2. При выборе B: `AppConfig.save_metadata = None` по умолчанию; `config.example.yaml` — строка закомментирована с пояснением; `docs/usage.md` и README — заметка об изменении; `import_from_jsonl` сохраняется для миграции.
3. Тесты конфигурации обновить под новое значение по умолчанию.

**Критерии приёмки.** Решение зафиксировано с обоснованием; при выборе B — изменение значения по умолчанию описано в README и usage, импорт из существующего JSONL продолжает работать.

**Примечание.** Единственная задача спринта, меняющая значение по умолчанию, — идёт последней и отдельным коммитом.

**Решение (2026-08-31): вариант B — JSONL опционален и выключен по умолчанию.** `save_metadata = None` в `AppConfig` и `DownloadOptions`; строка в `config.example.yaml` закомментирована с пояснением; README и `docs/usage.md` описывают изменение и путь возврата (`save_metadata: data/meta.jsonl` или `YTD_SAVE_METADATA`). Импорт из существующего JSONL не тронут, `tests/test_history_import.py` без изменений. Обновлены ожидания значений по умолчанию в `test_types.py` и `test_config.py`.

Побочный эффект: тесты больше не создают `data/meta.jsonl` в рабочем каталоге при запуске из корня репозитория.

---

## Группа 13 — Maintenance 1.4: совместимость и зависимости (2026-08-31)

Цель: сделать результат загрузки по умолчанию воспроизводимым на обычных плеерах
и вернуть зависимости в актуальное состояние.

### BL-1301 — Приоритет H.264/AVC1 в селекторах формата MP4

- **Gap:** — (эксплуатационная жалоба, не ревью) | **Design:** —
- **Priority:** P1 | **Status:** done (2026-08-31) | **Estimate:** M

**Описание.** Селекторы формата просили у yt-dlp «лучшее видео в MP4» без указания
видеокодека. YouTube для многих роликов отдаёт под этим условием AV1, и получившийся
MP4 не открывается штатными плеерами Windows, старыми телевизорами и частью
мобильных устройств — при формально корректном расширении файла. Дефолт `best`
в интерактивном меню (`bestvideo+bestaudio/best`) страдал сильнее всего: он не
ограничивал даже контейнер.

**Шаги.**
1. Ввести единый конструктор селектора `_video_format_selector(ext, aud_ext, max_h)`
   в `ytd/downloader.py` — чтобы правило жило в одном месте, а не в четырёх.
2. Для `ext == "mp4"` строить цепочку с приоритетом `vcodec^=avc1` и с деградацией
   до прежнего поведения, если H.264 в нужной высоте нет.
3. Для не-MP4 (`webm`) поведение сохранить без изменений.
4. Перевести на конструктор всех потребителей: `Downloader.build_ydl_opts`,
   `interactive.build_quality_options`, `workflows/playlist_unified._format_for_entry`.
5. Починить `extract_quality_suffix`: подстрока `bestaudio` внутри `bestvideo+bestaudio`
   ошибочно давала суффикс `_audio` видеофайлу.

**Критерии приёмки.**
- Селектор MP4 содержит `vcodec^=avc1` раньше generic-варианта той же высоты.
- Цепочка деградации сохраняет прежний результат там, где H.264 недоступен.
- Селектор WebM не изменился.
- Дефолт интерактивного меню даёт видео, а не суффикс `_audio`.

**Реализовано (2026-08-31).** Селектор для MP4 — шесть ступеней: H.264 в MP4 →
H.264 в любом контейнере → прогрессивный H.264 MP4 → любой MP4 → любой прогрессивный
MP4 → что угодно. Дефолт интерактивного меню переименован в «Лучшее совместимое MP4
качество» и получил суффикс `_best` вместо ошибочного `_audio`. Тесты:
`test_build_ydl_opts_video_best_mp4_prefers_h264`,
`test_build_ydl_opts_video_webm_keeps_webm_selector`,
`test_build_quality_options_prefers_h264_mp4_before_generic_mp4`.

**Перенос через рефакторинг.** Правка начиналась до спринта K и затрагивала
`execute_download`, вырезанный при декомпозиции (BL-1105). При переносе логика
переехала в `workflows/playlist_unified._format_for_entry`.

---

### BL-1302 — Обновление зависимостей, включая yt-dlp

- **Gap:** — | **Design:** —
- **Priority:** P2 | **Status:** done (2026-08-31) | **Estimate:** S

**Описание.** `uv.lock` фиксировал yt-dlp 2026.3.17 — пять месяцев без обновлений.
Для yt-dlp это критично: экстракторы ломаются вслед за изменениями площадок,
и старая версия со временем просто перестаёт скачивать.

**Шаги.**
1. `uv lock --upgrade`, разбор диффа локфайла.
2. Поднять нижнюю границу `yt-dlp` в `pyproject.toml`, чтобы пересборка окружения
   с нуля не могла взять старую версию.
3. Границы dev-инструментов не трогать: точные версии фиксирует локфайл, а свободный
   минимум упрощает сборку в чужом окружении.
4. Прогнать полный набор ворот качества и проверку на живой сети.

**Критерии приёмки.**
- `uv lock --check` и `uv sync --frozen` проходят (CI использует именно их).
- Тесты, покрытие, ruff и mypy зелёные на обновлённых версиях.
- CLI работает после мажорного шага typer.

**Реализовано (2026-08-31).** yt-dlp 2026.3.17 → **2026.8.19**; попутно typer
0.25.1 → 0.27.2, ruff 0.15.14 → 0.16.5, pytest 9.0.3 → 9.1.1, pygments 2.20.0 →
2.21.0, packaging 26.2 → 26.3, annotated-doc 0.0.4 → 0.0.5. Зависимость `click`
ушла из графа — typer 0.27 её больше не тянет; CLI и тесты Typer-слоя это
пережили без правок. Нижняя граница в `pyproject.toml`: `yt-dlp>=2026.8.19`.

**Замечание по окружению.** На машине разработчика `pypi.org/simple/` недоступен
(таймаут), при живом `files.pythonhosted.org`. Резолв выполнялся через зеркало
(`UV_INDEX_URL`). В файлы проекта зеркало намеренно не прописано: на GitHub Actions
`pypi.org` доступен, и навязывать зеркало CI и сторонним участникам не нужно —
это личная настройка машины (`~/.config/uv/uv.toml`).
...
| GAP-CR-039 | BL-1205 |
| GAP-CR-040 | BL-1206 |
| GAP-CR-009 (перенесён) | BL-1207 |

GAP-CR-001 … GAP-CR-025 — ревью [2026-05-25](./gaps/code_review_2026-05-25.md); GAP-CR-026 … GAP-CR-034 — ревью [2026-08-31](./gaps/code_review_2026-08-31.md); GAP-CR-035 … GAP-CR-040 — [ревью техдолга 2026-08-31](./gaps/tech_debt_2026-08-31.md).
...
- [x] P1 (добавлена по ходу спринта): BL-1110 — status `done`
...
- [x] P3: BL-1108, BL-1109 — status `done`
- [x] `docs/gaps/code_review_2026-08-31.md`: статусы GAP-CR-026 … GAP-CR-034 обновлены
- [x] `docs/gaps/README.md`: индекс синхронизирован

**Релиз Maintenance 1.2 закрыт 2026-08-31.** Все 10 задач спринта K (BL-1101 … BL-1110) — `done`, все 9 гэпов ревью 2026-08-31 — `fixed`. Финальное состояние: 121 тест (119 passed, 2 skipped), `ruff check .` чисто, CI зелёный на `ubuntu-latest` и `windows-latest`.

---

## Чеклист закрытия (Definition of Done для релиза «Maintenance 1.3»)

- [x] P1: BL-1201, BL-1202 — status `done`
- [x] P2: BL-1203, BL-1204, BL-1205 — status `done`
- [x] P3: BL-1206, BL-1207 — решения приняты и зафиксированы
- [x] Полный `pytest` зелёный на Windows и Linux; покрытие не ниже порога
- [x] `ruff check .` и проверка типов чисты
- [x] CI: `test (ubuntu-latest)`, `test (windows-latest)`, `lint` зелёные
- [x] `docs/gaps/tech_debt_2026-08-31.md`: статусы GAP-CR-035 … GAP-CR-040 и GAP-CR-009 обновлены
- [x] `docs/gaps/README.md`: индекс синхронизирован

**Релиз Maintenance 1.3 закрыт 2026-08-31.** Все 7 задач спринта L (BL-1201 … BL-1207) — `done`, все 7 гэпов ревью техдолга — `fixed`. Финальное состояние: 129 тестов (127 passed, 2 skipped), покрытие 68% при пороге 66%, mypy зелёный под тремя платформенными моделями, `ruff check .` чисто, ветка `main` защищена обязательными проверками.

---

## Чеклист закрытия (Definition of Done для релиза «Maintenance 1.4»)

- [x] P1: BL-1301 — status `done`
- [x] P2: BL-1302 — status `done`
- [x] Полный `pytest` зелёный; покрытие не ниже порога
- [x] `ruff check .` и проверка типов чисты на обновлённых версиях инструментов
- [x] `uv lock --check` и `uv sync --frozen` проходят
- [x] Проверка на живой сети: `info`, `download`, подтверждение кодека через `ffprobe`

**Релиз Maintenance 1.4 закрыт 2026-08-31.** Обе задачи спринта M — `done`. Финальное состояние: 132 теста (130 passed, 2 skipped), покрытие 67.63% при пороге 66%, mypy 2.3.1 зелёный под тремя платформенными моделями, `ruff check .` чисто на ruff 0.16.5, yt-dlp обновлён до 2026.8.19. Спринт не опирался на гэп-ревью: поводом были эксплуатационная жалоба на воспроизводимость MP4 и возраст основной зависимости.
...
- [Гэп-ревью 2026-08-31](./gaps/code_review_2026-08-31.md) — GAP-CR-026 … GAP-CR-034 (closed)
- [Дизайн исправлений 2026-08-31](./design_2026-08-31.md) — D-1 … D-7 (реализовано)
- [Ревью техдолга 2026-08-31](./gaps/tech_debt_2026-08-31.md) — GAP-CR-035 … GAP-CR-040
- [Дизайн исправлений техдолга](./design_tech_debt_2026-08-31.md) — D-1 … D-7 (спринт L)
