...
def _fetch_one_row(
    ...
    query = f"SELECT * FROM downloads WHERE {where_clause} ORDER BY finished_at DESC NULLS LAST"
    try:
        return conn.execute(query, params).fetchone()
    except sqlite3.OperationalError:
        fallback_query = query.replace(" ORDER BY finished_at DESC NULLS LAST", " ORDER BY finished_at DESC")
        return conn.execute(fallback_query, params).fetchone()


class HistoryStore:
    """Хранилище истории загрузок с явным путём к SQLite."""
    ...
    def record_event(self, event: DownloadEvent) -> None:
        ...
        if event.metadata and event.metadata_path:
            try:
                ...
                logging.getLogger("ytd.history").warning(
                    "не удалось сохранить метаданные для %s в %s: %s",
                    payload.get("video_id") or event.url,
                    event.metadata_path,
                    exc,
                )

    def import_from_jsonl(self, path: Path | str) -> int:
    ...
    def fetch_download(
        ...
        with closing(self.get_connection()) as conn:
            if video_id:
                row = _fetch_one_row(conn, "video_id = :video_id", {"video_id": video_id})
                if row is not None:
                    return _row_to_dict(row)
            if url:
                row = _fetch_one_row(conn, "url = :url", {"url": url})
                if row is not None:
                    return _row_to_dict(row)
        return None
    ...
    def update_download(
        ...
        *,
        ...
        with closing(self.get_connection()) as conn:
            ...
            matched = False
            if video_id:
                query = f"UPDATE downloads SET {set_clause} WHERE video_id = :video_id"
                params_with_id = {**params, "video_id": video_id}
                cursor = conn.execute(query, params_with_id)
                matched = cursor.rowcount > 0
            if not matched and url:
                query = f"UPDATE downloads SET {set_clause} WHERE url = :url"
                params_with_url = {**params, "url": url}
                conn.execute(query, params_with_url)
            conn.commit()
...
def set_default_store(store: HistoryStore | None) -> None:
    global _default_store
    _default_store = store


def init_db(path: Path | str) -> Path:
    """Установить путь к файлу БД истории загрузок и store по умолчанию."""
...
def get_connection() -> sqlite3.Connection:
    """Получить соединение с SQLite-базой истории (default store)."""
    return get_default_store().get_connection()


def ensure_schema() -> bool:
    return get_default_store().ensure_schema()


def record_event(event: DownloadEvent) -> None:
    get_default_store().record_event(event)


def import_from_jsonl(path: Path | str) -> int:
    return get_default_store().import_from_jsonl(path)


def fetch_download(video_id: str | None = None, url: str | None = None) -> dict[str, Any] | None:
    """Получить запись из истории по video_id или URL (default store)."""
    return get_default_store().fetch_download(video_id=video_id, url=url)
...
def update_download(
    ...
    *,
    ...
    get_default_store().update_download(
        video_id=video_id,
        url=url,
        status=status,
...
def list_downloads(
    *,
    ...
    return get_default_store().list_downloads(
        statuses=statuses,
        limit=limit,
        since=since,
        playlist_id=playlist_id,
...
def _extract_timestamp(raw: Any) -> str | None:
    candidates: list[Any] = []
...
def _extract_youtube_id(candidate: str) -> str | None:
    if not candidate:
        return None
