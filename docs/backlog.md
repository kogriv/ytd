...
**Решение (2026-08-31): вариант A — защита с обязательными проверками и PR-потоком.** Обоснование: за два цикла сопровождения в `main` ушло девять прямых пушей, каждый проверялся уже постфактум; CI сообщал, но ничего не гарантировал. Правило: обязательны `test (ubuntu-latest)`, `test (windows-latest)`, `lint`; требуется PR перед merge; force-push и удаление ветки запрещены; ревью не требуется (работа одиночная); `enforce_admins` не включён, чтобы владелец мог снять правило при срочной необходимости. Поток описан в `DEVELOPMENT.md`.
